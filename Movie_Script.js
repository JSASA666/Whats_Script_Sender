...
async function enviarScript(scriptText){
	...
	if(!textarea) throw new Error("No conversation")
	...
	for(const line of lines){
		...
		setTimeout(() => {
			(main.querySelector(`button[aria-label='Send']`) || main.querySelector(`[data-icon="wds-ic-send-filled"]`)).click(); //Part to change is sender is "null" or not working
		}, 100);
		
		if(lines.indexOf(line) !== lines.length - 1) await new Promise(resolve => setTimeout(resolve, 250)); //Waiting time between lines
	}
	
	return lines.length;
}
// Script to send to send in WhatsApp Web
enviarScript(`
...
SAW
               ...
               with Lawrence's two-faced bullshit.
                         ...
                         Lawrence at an outdoor caf' eating lunch.)
               ...
               THE END"`).then(e => console.log(`Code finished, ${e} sent messages`)).catch(console.error);
